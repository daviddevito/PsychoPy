...
outStr = "SubjectNumber\tAge\tGender\tHandedness\tGlassesOrContacts\tTime\tTrialType\tTrial#\tCuedType(0=Both,1=CuedSide,2=CuedEmotion,3=UncuedTrial)\tCuedSide(0=Left,1=Right)\tEmotion(0=Happy,1=Angry)\tArrowColor\tFaceStim\tFacePosition\tCorrectKey\tKeyResponse\tKeyResponseRT\tErrorCode\tErrorName\tRegularOrCatch(0=Regular Trial,1=Opposite Emotion Catch Trial,2=Same Emotion Catch Trial)\t"
...
for ptrial in range(0, 300):
    ...
    keyResp.keys = []
    keyResp.rt = -1
    while trialClock.getTime() < INTER_TRIAL_INTERVAL + CUEARROW_TIME + FACE_ON_SCREEN + FACE_OFF_SCREEN:
    ...
    feedbackText.setAutoDraw(True);win.flip();

    while trialClock.getTime() < INTER_TRIAL_INTERVAL + CUEARROW_TIME + FACE_ON_SCREEN + FACE_OFF_SCREEN + FEEDBACK_TIME:pass
